  ...
  WITH
      ...
      t_player_games_on_real_money_with_ids AS (
        SELECT
          *,
          LAG(f_ended_stamp)
          OVER w AS prev_ended_stamp,
          (CASE
           WHEN diff_in_minutes(f_started_stamp, LAG(f_ended_stamp)
           OVER w) <= 30
             THEN NULL
           ELSE 1
      ...
      t_player_games_with_sessions_ids AS (
        SELECT
          *,
          ...
          OVER (
            ORDER BY f_player_id, f_started_stamp, f_ended_stamp,
              f_table_type) AS f_session_id
        FROM
