CREATE MATERIALIZED VIEW mv_macro AS (
    ...
    registrations AS (
        ...
        FROM (
                ...
                COUNT(tt.f_id) + (
                    SELECT 
                        COUNT(DISTINCT f_id) AS client_base
                    FROM
    ...
    daily_macro_base_metrics AS (
        SELECT 
            ...
            COALESCE(room_balance.room_balance, 0) AS room_balance,
            COALESCE(bot_balance.bot_balance, 0) AS bot_balance,
            COALESCE(house_balance.house_balance, 0) AS house_balance,

            COALESCE(bot_cash_profit.bot_cash_profit, 0) AS bot_cash_profit,
            COALESCE(house_cash_profit.house_cash_profit, 0) AS house_cash_profit,
            COALESCE(bot_tour_profit.bot_tour_profit, 0) AS bot_tour_profit,
            COALESCE(house_tour_profit.house_tour_profit, 0) AS house_tour_profit,

            COALESCE(cash_rake.cash_rake, 0) AS cash_rake,
            COALESCE(bot_cash_rake.bot_cash_rake, 0) AS bot_cash_rake,
            COALESCE(house_cash_rake.house_cash_rake, 0) AS house_cash_rake,

            COALESCE(tournament_rake.tournament_rake, 0) AS tournament_rake,
            COALESCE(bot_tournament_rake.bot_tournament_rake, 0) AS bot_tournament_rake,
            COALESCE(house_tournament_rake.house_tournament_rake, 0) AS house_tournament_rake
        FROM dau
    ...
    daily_macro_with_cumulative_base_metrics AS (
        SELECT
            ...
            SUM(deposit) OVER w AS cumulative_deposit,
            SUM(cashout) OVER w AS cumulative_cashout,

            SUM(balance) OVER w AS cumulative_balance,
            SUM(room_balance) OVER w AS cumulative_room_balance,
            SUM(bot_balance) OVER w AS cumulative_bot_balance,
            SUM(house_balance) OVER w AS cumulative_house_balance,

            SUM(bot_cash_profit) OVER w AS cumulative_bot_cash_profit,
            SUM(house_cash_profit) OVER w AS cumulative_house_cash_profit,
            
            SUM(bot_tour_profit) OVER w AS cumulative_bot_tour_profit,
            SUM(house_tour_profit) OVER w AS cumulative_house_tour_profit,
            
            SUM(cash_rake) OVER w AS cumulative_cash_rake,
            SUM(bot_cash_rake) OVER w AS cumulative_bot_cash_rake,
            SUM(house_cash_rake) OVER w AS cumulative_house_cash_rake,
            
            SUM(total_rake) OVER w AS cumulative_total_rake,
            SUM(bot_rake) OVER w AS cumulative_bot_rake,
            SUM(live_rake) OVER w AS cumulative_live_rake,

            SUM(tournament_rake) OVER w AS cumulative_tournament_rake,
            SUM(bot_tournament_rake) OVER w AS cumulative_bot_tournament_rake,
            SUM(house_tournament_rake) OVER w AS cumulative_house_tournament_rake
        FROM
    ...
    daily_macro_with_extra_cumulative_metrics AS (
        SELECT 
            *,
            (cumulative_bot_cash_profit + cumulative_bot_tour_profit) AS cumulative_bot_profit,
            (cumulative_house_cash_profit + cumulative_house_tour_profit) AS cumulative_house_profit,
            (cumulative_balance - cumulative_bot_balance) AS cumulative_live_balance,
            (cumulative_room_balance + cumulative_bot_balance) AS cumulative_revenue
        FROM 
            daily_macro_with_cumulative_base_metrics
    )
    SELECT 
        *,
        (cumulative_bot_profit + cumulative_bot_rake) AS cumulative_bot_profit_and_rake
    FROM
