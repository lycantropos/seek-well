CREATE MATERIALIZED VIEW mv_pool_sng_texas_games_count AS (
  WITH
      ...
      live_players AS (
        SELECT
          DATE_TRUNC('month', f_date) :: DATE AS f_date,
          SUM(
              f_games_count_sng_texas)        AS f_games_count_sng_texas_live_players
        FROM
          mv_real_money_games_count
          JOIN
          t_house_flag USING (f_player_id)
      ...
      house_players AS (
        SELECT
          DATE_TRUNC('month', f_date) :: DATE AS f_date,
          SUM(
              f_games_count_sng_texas)        AS f_games_count_sng_texas_house_players
        FROM
          mv_real_money_games_count
          JOIN
          t_house_flag USING (f_player_id)
      ...
      test_players AS (
        SELECT
          DATE_TRUNC('month', f_date) :: DATE AS f_date,
          SUM(
              f_games_count_sng_texas)        AS f_games_count_sng_texas_test_players
        FROM
          mv_real_money_games_count
          JOIN
          t_house_flag USING (f_player_id)
  ...
  SELECT
    ...
    COALESCE(f_games_count_sng_texas_all,
             0)                                        AS f_games_count_sng_texas_all,
    COALESCE(f_games_count_sng_texas_bots,
             0)                                        AS f_games_count_sng_texas_bots,
    COALESCE(f_games_count_sng_texas_live_players,
             0)                                        AS f_games_count_sng_texas_live_players,
    COALESCE(f_games_count_sng_texas_house_players,
             0)                                        AS f_games_count_sng_texas_house_players,
    COALESCE(f_games_count_sng_texas_test_players,
             0)                                        AS f_games_count_sng_texas_test_players
  FROM t_all
