CREATE MATERIALIZED VIEW mv_pool_sng_tournaments AS (
  WITH
      ...
      live_players AS (
        SELECT
          DATE_TRUNC('month', f_started_stamp) :: DATE AS f_date,
          COUNT(DISTINCT
              tour.f_id)                               AS f_sng_touraments_live_players
        FROM
          player_in_sng_tournaments AS tour,
          t_house_flag AS hf
        WHERE
      ...
      house_players AS (
        SELECT
          DATE_TRUNC('month', f_started_stamp) :: DATE AS f_date,
          COUNT(DISTINCT
              tour.f_id)                               AS f_sng_touraments_house_players
        FROM
          player_in_sng_tournaments AS tour,
          t_house_flag AS hf
        WHERE
      ...
      test_players AS (
        SELECT
          DATE_TRUNC('month', f_started_stamp) :: DATE AS f_date,
          COUNT(DISTINCT
              tour.f_id)                               AS f_sng_touraments_test_players
        FROM
          player_in_sng_tournaments AS tour,
          t_house_flag AS hf
        WHERE
  ...
  SELECT
    ...
    COALESCE(f_sng_touraments_live_players,
             0)                                 AS f_sng_touraments_live_players,
    COALESCE(f_sng_touraments_house_players,
             0)                                 AS f_sng_touraments_house_players,
    COALESCE(f_sng_touraments_test_players,
             0)                                 AS f_sng_touraments_test_players
  FROM t_all
