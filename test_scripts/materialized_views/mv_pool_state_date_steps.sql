CREATE MATERIALIZED VIEW mv_pool_state_date_steps AS (
    ...
    player_cumulative_balance_by_day_with_null AS (
        SELECT
            ...
            COUNT(f_cumulative_balance) OVER w AS f_cumulative_balance_id,
    ...
    player_cumulative_balance_by_day AS (
        SELECT
            ...
            COALESCE(
                f_cumulative_balance,
                first_value(f_cumulative_balance) OVER w,
                0
            ) AS f_value
        FROM
    ...
    player_cumulative_deposit_by_day_with_null AS (
        SELECT
            ...
            COUNT(f_cumulative_deposit) OVER w AS f_cumulative_deposit_id,
    ...
    player_cumulative_deposit_by_day AS (
        SELECT
            ...
            COALESCE(
                f_cumulative_deposit,
                first_value(f_cumulative_deposit) OVER w,
                0
            ) AS f_value
        FROM
    ...
    player_cumulative_cashouts AS (
        SELECT
            f_player_id,
            f_date,
            SUM(ABS(f_cashouts)) OVER w AS f_cumulative_cashouts
        FROM
            v_cashouts
        JOIN
            live_player_with_registration USING(f_player_id)
    ...
    player_cumulative_cashouts_by_day_with_null AS (
        SELECT
            ...
            COUNT(f_cumulative_cashouts) OVER w AS f_cumulative_cashouts_id,
    ...
    player_cumulative_cashouts_by_day AS (
        SELECT
            ...
            COALESCE(
                f_cumulative_cashouts,
                first_value(f_cumulative_cashouts) OVER w,
                0
            ) AS f_value
        FROM
    ...
    player_cumulative_rake_by_day_with_null AS (
        SELECT
            ...
            f_date,
            f_cumulative_rake,
            COUNT(f_cumulative_rake) OVER w AS f_cumulative_rake_id,
    ...
    player_cumulative_rake_by_day AS (
        SELECT
            ...
            COALESCE(
                f_cumulative_rake,
                first_value(f_cumulative_rake) OVER w,
                0
            ) AS f_value
        FROM
    ...
    all_registrations_dates AS (
        SELECT
            generate_series(min(f_register_date), now(), '1 day')::DATE AS f_date
        FROM
            live_player_with_registration
    ),
    dates_with_steps AS (
        SELECT
            f_date,
            f_step,
            (f_date + CONCAT(f_step, ' days')::INTERVAL)::DATE AS f_date_with_step
        FROM
    ...
    metrics_with_zero_step AS (
        ...
        FROM
            ...
            (
                SELECT * FROM player_cumulative_balance_by_day
                UNION ALL
                SELECT * FROM player_cumulative_deposit_by_day
                UNION ALL
                SELECT * FROM player_cumulative_cashouts_by_day
                UNION ALL
                SELECT * FROM player_cumulative_rake_by_day
            ) AS metrics
        WHERE
            dates.f_step = 0 AND
    ...
    SELECT 
        *,
        f_date::VARCHAR(20) AS f_string_date, -- for filtering in superset
        f_step::VARCHAR(10) AS f_string_step -- for filtering in superset
    FROM (
        SELECT * FROM metrics_with_zero_step
        UNION ALL
        SELECT * FROM metrics_with_not_zero_step
    ) AS all_metrics
)