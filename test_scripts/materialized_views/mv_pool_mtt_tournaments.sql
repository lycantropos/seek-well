CREATE MATERIALIZED VIEW mv_pool_mtt_tournaments AS (
  WITH
      ...
      live_players AS (
        SELECT
          DATE_TRUNC('month', f_started_stamp) :: DATE AS f_date,
          COUNT(DISTINCT
              tour.f_id)                               AS f_mtt_touraments_live_players
        FROM
          player_in_mtt_tournaments AS tour,
          t_house_flag AS hf
        WHERE
      ...
      house_players AS (
        SELECT
          DATE_TRUNC('month', f_started_stamp) :: DATE AS f_date,
          COUNT(DISTINCT
              tour.f_id)                               AS f_mtt_touraments_house_players
        FROM
          player_in_mtt_tournaments AS tour,
          t_house_flag AS hf
        WHERE
      ...
      test_players AS (
        SELECT
          DATE_TRUNC('month', f_started_stamp) :: DATE AS f_date,
          COUNT(DISTINCT
              tour.f_id)                               AS f_mtt_touraments_test_players
        FROM
          player_in_mtt_tournaments AS tour,
          t_house_flag AS hf
        WHERE
  ...
  SELECT
    ...
    COALESCE(f_mtt_touraments_live_players,
             0)                                 AS f_mtt_touraments_live_players,
    COALESCE(f_mtt_touraments_house_players,
             0)                                 AS f_mtt_touraments_house_players,
    COALESCE(f_mtt_touraments_test_players,
             0)                                 AS f_mtt_touraments_test_players
  FROM t_all
