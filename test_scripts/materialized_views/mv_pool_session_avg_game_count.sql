CREATE MATERIALIZED VIEW mv_pool_session_avg_game_count AS (
    ...
    live_players AS (
        SELECT 
            DATE_TRUNC('month', f_date)::DATE AS f_date,
            AVG(f_games_count) AS f_session_avg_game_count_live_players
        FROM 
            mv_player_sessions
         JOIN 
            t_house_flag USING(f_player_id)
    ...
    house_players AS (
        SELECT 
            DATE_TRUNC('month', f_date)::DATE AS f_date,
            AVG(f_games_count) AS f_session_avg_game_count_house_players
        FROM 
            mv_player_sessions
         JOIN 
            t_house_flag USING(f_player_id)
    ...
    test_players AS (
        SELECT 
            DATE_TRUNC('month', f_date)::DATE AS f_date,
            AVG(f_games_count) AS f_session_avg_game_count_test_players
        FROM 
            mv_player_sessions
         JOIN 
            t_house_flag USING(f_player_id)
