CREATE MATERIALIZED VIEW mv_pool_session_count AS (
    WITH
    t_all AS (
        SELECT
            DATE_TRUNC('month', f_date)::DATE AS f_date,
            COUNT(*) AS f_session_count_all
        FROM 
            mv_player_sessions
        GROUP BY
    ...
    live_players AS (
        SELECT
            DATE_TRUNC('month', f_date)::DATE AS f_date,
            COUNT(*) AS f_session_count_live_players
        FROM 
            mv_player_sessions
        JOIN 
            t_house_flag USING(f_player_id)
    ...
    house_players AS (
        SELECT
            DATE_TRUNC('month', f_date)::DATE AS f_date,
            COUNT(*) AS f_session_count_house_players
        FROM 
            mv_player_sessions
        JOIN 
            t_house_flag USING(f_player_id)
    ...
    test_players AS (
        SELECT
            DATE_TRUNC('month', f_date)::DATE AS f_date,
            COUNT(*) AS f_session_count_test_players
        FROM 
            mv_player_sessions
        JOIN 
            t_house_flag USING(f_player_id)
    ...
    SELECT
        ...
        COALESCE(f_session_count_all, 0) AS f_session_count_all,
        COALESCE(f_session_count_bots, 0) AS f_session_count_bots,
        COALESCE(f_session_count_live_players, 0) AS f_session_count_live_players,
        COALESCE(f_session_count_house_players, 0) AS f_session_count_house_players,
        COALESCE(f_session_count_test_players, 0) AS f_session_count_test_players
    FROM t_all
