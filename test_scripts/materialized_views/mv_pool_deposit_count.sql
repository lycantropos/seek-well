CREATE MATERIALIZED VIEW mv_pool_deposit_count AS (
    ...
    all_dates AS (
        SELECT
            generate_series(min(f_date), max(f_date), '1 month')::DATE AS f_date
        FROM 
            t_all
    ),
    with_gaps AS (
        SELECT
            ...
            COALESCE(f_deposit_count_live_players, 0) AS f_deposit_count_live_players,
            COALESCE(f_deposit_count_house_players, 0) AS f_deposit_count_house_players,
            COALESCE(f_deposit_count_test_players, 0) AS f_deposit_count_test_players
        FROM t_all
    ...
    cumulative_sum AS (
        SELECT
            f_date,
            SUM(f_deposit_count_all) OVER w AS f_deposit_count_all,
            SUM(f_deposit_count_bots) OVER w AS f_deposit_count_bots,
            SUM(f_deposit_count_live_players) OVER w AS f_deposit_count_live_players,
            SUM(f_deposit_count_house_players) OVER w AS f_deposit_count_house_players,
            SUM(f_deposit_count_test_players) OVER w AS f_deposit_count_test_players
        FROM
            with_gaps
        WINDOW w AS (
            ORDER BY f_date
    ...
    with_ids AS (
        SELECT
            ...
            f_deposit_count_all,
            COUNT(f_deposit_count_all) OVER w AS f_deposit_count_all_id,
            
            f_deposit_count_bots,
            COUNT(f_deposit_count_bots) OVER w AS f_deposit_count_bots_id,
        
            f_deposit_count_live_players,
            COUNT(f_deposit_count_live_players) OVER w AS f_deposit_count_live_players_id,
        
            f_deposit_count_house_players,
            COUNT(f_deposit_count_house_players) OVER w AS f_deposit_count_house_players_id,
        
            f_deposit_count_test_players,
            COUNT(f_deposit_count_test_players) OVER w AS f_deposit_count_test_players_id
        FROM cumulative_sum
        WINDOW w AS (
            ORDER BY f_date
        )
    ...
    without_gaps AS (
        SELECT
            ...
            COALESCE(f_deposit_count_all, first_value(f_deposit_count_all) 
                OVER (PARTITION BY f_deposit_count_all_id ORDER BY f_date)
            ) AS f_deposit_count_all,
            
            COALESCE(f_deposit_count_bots, first_value(f_deposit_count_bots) 
                OVER (PARTITION BY f_deposit_count_bots_id ORDER BY f_date)
            ) AS f_deposit_count_bots,
        
            COALESCE(f_deposit_count_live_players, first_value(f_deposit_count_live_players) 
                OVER (PARTITION BY f_deposit_count_live_players_id ORDER BY f_date)
            ) AS f_deposit_count_live_players, 
        
            COALESCE(f_deposit_count_house_players, first_value(f_deposit_count_house_players) 
                OVER (PARTITION BY f_deposit_count_house_players_id ORDER BY f_date)
            ) AS f_deposit_count_house_players, 
        
            COALESCE(f_deposit_count_test_players, first_value(f_deposit_count_test_players) 
                OVER (PARTITION BY f_deposit_count_test_players_id ORDER BY f_date)
