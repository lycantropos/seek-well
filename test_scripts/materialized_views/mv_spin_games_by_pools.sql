CREATE MATERIALIZED VIEW mv_spin_games_by_pools AS (

    WITH
    last_game AS (
        SELECT 
            MAX(g.f_started_stamp) AS last_game_stamp
        FROM
    ...
    all_games_filtered AS (
        SELECT
            g.f_id,
            ((tour.f_buy_in + tour.f_entry_fee)::FLOAT / 100) AS f_buy_in,
            DATE_TRUNC('hour', g.f_started_stamp) AS f_stamp,
            p.f_player_id,
            (SUM(COALESCE(tr.f_value, 0)::NUMERIC / 100)) AS f_rake -- in dollars
        FROM
            last_game AS lg,
            t_participation AS p,
            t_game AS g,
        ...
        WHERE
            g.f_started_stamp >= last_game_stamp - INTERVAL '7 days' AND
            p.f_game_id = g.f_id AND
            g.f_table_id = t.f_id AND
            t.f_tournament_id = tour.f_id AND
            tour.f_money_type='R' AND
        ...
        GROUP BY
            g.f_id,
            ((tour.f_buy_in + tour.f_entry_fee)::FLOAT / 100),
    ...
    SELECT
        f_buy_in::VARCHAR(20), -- For filterbox in superset
    ...
    FULL JOIN 
        games_with_bots_count AS b USING (f_id, f_buy_in, f_stamp)
    GROUP BY
        f_buy_in,
        f_bots_count,
        f_players_count
